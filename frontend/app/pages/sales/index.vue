    ...
    <header class="shrink-0 border-b border-gray-200 bg-white px-6 py-5">
      <div class="flex items-center justify-between gap-4">
        <div>
          ...
          <h1 class="mt-2 text-2xl font-semibold">Sales Task</h1>
          <p class="mt-1 text-sm text-gray-500">Monitor tasks assigned to sales users.</p>
        </div>
      ...
      <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <table class="min-w-full text-left">
          <thead class="bg-gray-50">
            <tr>
              <th class="px-5 py-4 text-sm font-semibold">Task</th>
              <th class="px-5 py-4 text-sm font-semibold">Assignee</th>
              <th class="px-5 py-4 text-sm font-semibold">Status</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="task in tasks" :key="task.id" class="border-t border-gray-100">
              <td class="px-5 py-4">{{ task.title }}</td>
              <td class="px-5 py-4 text-gray-600">{{ task.assignee }}</td>
              <td class="px-5 py-4 text-gray-600">{{ task.status }}</td>
            </tr>
...
onMounted(() => {
  try {
    tasks.value = JSON.parse(localStorage.getItem("sales-task-records") || "[]");
  } catch {
    tasks.value = [];
  }
});
