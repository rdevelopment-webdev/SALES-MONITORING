...
// Prospect Technique
const prospectTechniqueOptions = ref([{ id: 1, name: "" }]);
...
// Static service options that should always be present, even if the
// backend lookup endpoint overwrites the list below.
...
// ============================================
// ADD NEW PROSPECT TECHNIQUE (inline, from Prospect Technique dropdown)
// ============================================
...
function openAddTechnique() {
  ...
  nextTick(() => newTechniqueInputRef.value?.focus());
}

function cancelAddTechnique() {
  showAddTechniqueInput.value = false;
...
// ============================================
// ADD NEW WAY OF COMMUNICATION (inline, from Ways of Communication dropdown)
// ============================================
...
function openAddCommunication() {
  ...
  nextTick(() => newCommunicationInputRef.value?.focus());
}

function cancelAddCommunication() {
  showAddCommunicationInput.value = false;
...
async function loadLookups() {
  ...
  try {
    ...
    if (lookups.techniques?.length)
      prospectTechniqueOptions.value = lookups.techniques;
    if (lookups.salesReps?.length) salesRepOptions.value = lookups.salesReps;
    if (lookups.services?.length) {
      const existingNames = new Set(
        lookups.services.map((s) => s.name.toLowerCase())
      );
      const missingStatics = staticServiceOptions.filter(
        (s) => !existingNames.has(s.name.toLowerCase())
      );
      serviceOptions.value = [...lookups.services, ...missingStatics];
    }
    if (lookups.communications?.length)
      communicationOptions.value = lookups.communications;
    if (lookups.locations?.length) locationOptions.value = lookups.locations;
  } catch (error) {
    console.warn(
      "Failed to fetch lookup options, using local fallback:",
      error
    );
  } finally {
...
async function saveRecord() {
  ...
  try {
    ...
    toast.success(`${form.value.clientName} was created successfully.`);
