      ...
      <div class="px-4 py-2.5 bg-white">
        <div class="grid grid-cols-2 gap-x-5 gap-y-2">
          ...
          <div class="space-y-2">
            ...
            <div>
              <label class="block text-[10.5px] font-bold text-[#1F2835] mb-0.5"
                >Prospect Technique</label
              >
              <div class="relative dropdown-wrapper">
                <button
                  type="button"
                  @click="toggleTechniqueDropdown"
                  :class="[
                    'w-full bg-white border border-gray-300 rounded-[4px] px-2.5 py-[5px] pr-7 text-[11px] focus:outline-none focus:border-[#F52C11] transition-colors text-left h-[28px] truncate',
                ...
                <svg
                  :class="[
                    'w-2.5 h-2.5 text-[#F52C11] absolute right-2.5 top-[9px] pointer-events-none transition-transform',
                ...
                >
                  <div
                    ...
                    :class="[
                      'px-2.5 py-1.5 text-[11px] cursor-pointer transition-colors',
          ...
          <div class="space-y-2">
              ...
              <div class="relative dropdown-wrapper">
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        class="text-[#f5211] hover:text-[#d9250e]"
                      >
                        <svg
                          class="w-2.5 h-2.5"
                          fill="none"
                ...
                >
                  <div
                    ...
                    class="flex items-center gap-2 px-2.5 py-1.5 hover:bg-[#F52C11]/5 cursor-pointer transition-colors"
                  >
                    <div
                      :class="[
            ...
            <div>
              <label class="block text-[10.5px] font-bold text-[#1F2835] mb-0.5"
                >Ways of Communication</label
              >
              <div class="relative dropdown-wrapper">
                <button
                  type="button"
                  @click="toggleCommunicationDropdown"
                  :class="[
                    'w-full bg-white border border-gray-300 rounded-[4px] px-2.5 py-[5px] pr-7 text-[11px] focus:outline-none focus:border-[#F52C11] transition-colors text-left h-[28px] truncate',
        ...
        <div class="grid grid-cols-2 gap-x-5 mt-3">
          ...
          >
            <!-- Card Header -->
            <div
              class="flex items-center gap-1.5 px-3 py-1.5 border-b border-gray-100"
            >
              <div
            ...
            <div class="px-3 pb-2.5 pt-1 space-y-1.5">
              <div class="flex items-center gap-1.5">
                <div
                  class="w-7 h-7 rounded-[4px] bg-gray-50 flex items-center justify-center shrink-0"
                >
                  <svg
          ...
          >
            ...
            <div class="px-3 pb-2.5 pt-1">
              <div
                class="flex items-start justify-between mb-0.5 border-b border-gray-100 pb-2"
              >
        ...
        <div
          v-if="saveError"
          class="flex items-start gap-2 px-3 py-2 mt-2 rounded-[4px] bg-[#F52C11]/10 border border-[#F52C11]/30 text-[#F52C11] text-[10.5px]"
        >
          <svg
...
function initializeForm() {
  ...
  Object.assign(form, {
    ...
    prospectTechnique:
      source.prospectTechniqueId ?? source.prospectTechnique ?? "",
    salesRepresentative: Array.isArray(source.salesRepresentativeIds)
      ? [...source.salesRepresentativeIds]
      : Array.isArray(source.salesRepresentative)
      ? [...source.salesRepresentative]
      : [],
    email: source.email || "",
    waysOfCommunication:
      source.waysOfCommunicationId ?? source.waysOfCommunication ?? "",
...
function toggleSalesRepDropdown() {
  const opening = !showSalesRepDropdown.value;
  closeAllDropdowns();
  showSalesRepDropdown.value = opening;
}
...
// ============================================
// PROSPECT TECHNIQUE / SERVICE / WAYS OF COMMUNICATION
// Single-select custom dropdowns (replace native <select> so the option
// list can be fully styled — native <select> popups ignore CSS for their
// hover/selected highlight and always render the OS system accent color).
// ============================================
function toggleTechniqueDropdown() {
  const opening = !showTechniqueDropdown.value;
  closeAllDropdowns();
  showTechniqueDropdown.value = opening;
}

function toggleServiceDropdown() {
  const opening = !showServiceDropdown.value;
  closeAllDropdowns();
  showServiceDropdown.value = opening;
}

function toggleCommunicationDropdown() {
  const opening = !showCommunicationDropdown.value;
  closeAllDropdowns();
  showCommunicationDropdown.value = opening;
}

function selectTechnique(id) {
  form.prospectTechnique = id;
  showTechniqueDropdown.value = false;
}
...
function selectCommunication(id) {
  form.waysOfCommunication = id;
  showCommunicationDropdown.value = false;
}

const techniqueLabel = computed(
  () =>
    props.prospectTechniqueOptions.find((t) => t.id === form.prospectTechnique)
      ?.name || ""
);

const serviceLabel = computed(
  () => props.serviceOptions.find((s) => s.id === form.service)?.name || ""
);

const communicationLabel = computed(
  () =>
    props.communicationOptions.find((w) => w.id === form.waysOfCommunication)
      ?.name || ""
);
